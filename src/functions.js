...
{
  var returnArray = new Array(logArray.length);

  for (i = 0; i < logArray.length; i++)
  {
    returnArray[i] = new GitLog;
    logParts = logArray[i].split(' ');

    returnArray[i].hash = logParts[0];

    returnArray[i].date = '';
    for (j = 1; j < 6; j++)
    {
      returnArray[i].date += logParts[j];
      returnArray[i].date += ' ';
    }
    returnArray[i].date += logParts[6];
    returnArray[i].date = new Date(returnArray[i].date);

    returnArray[i].message = logArray[i].split('\"')[1];
